...
public abstract class AStarTraversal extends GraphTraversal{
    ...
    public AStarTraversal(Tile[][] grid){
		super();
		this.grid = grid; 
    ...
    public List<Node> expandFunc(Node n) {
		...
		//create the potential next position
			...
			if (inPath(next,path) || !next.open(grid)  || !gridTypeOkay(grid[nextX][nextY]))
				continue;
			//printCurrentList();
	...
	public abstract boolean gridTypeOkay(Tile t);
}